...
#define MAX_SEQ 63 //帧的序号空间，应当是2^n-1
#define NR_BUFS 32 //窗口大小，NR_BUFS=(MAX_SEQ+1)/2
...
static void put_frame(unsigned char *frame, int len)
{
	...
	*(unsigned int *)(frame + len) = crc32(frame, len);
	...
	phl_ready = false; //将数据发送到物理层后将物理层置为忙碌状态
}
...
static void send_data_frame(unsigned char frame_nr)
{
	...
	s.kind = FRAME_DATA;
	s.seq = frame_nr;
	s.ack = (frame_expected + MAX_SEQ) & MAX_SEQ;
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < NR_BUFS; i++) //初始时没有输入帧被缓存
	{
		arrived[i] = false;
	}
		...
		case FRAME_RECEIVED:								 //当物理层收到一帧时
			...
			if (f.kind == FRAME_DATA) //如果是data帧
			{
					...
					//如果收到的帧在接收方窗口内且这一帧未被接收过
				{
					...
					{
						put_packet(in_buffer[frame_expected % NR_BUFS], PKT_LEN); //将输入缓存送至网络层
						no_nak = true;
			...
			if ((f.kind == FRAME_NAK) && between(ack_expected, (f.ack + 1) & MAX_SEQ, next_frame_to_send))
